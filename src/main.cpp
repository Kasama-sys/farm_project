...
Arduino_GFX *gfx = new Arduino_ILI9342(bus, 4 /* RST */, 0 /*rotation*/, false /*IPS*/);
#define CANVAS
...
DHT dht(pinDHT, DHT22);      // ← must come BEFORE...
BH1750 lightMeter;           // ← ...this too

int soilPinNum  = pinSoil;
int lightPinNum = pinLight;

Soil_pin  soilpin  = { &soilPinNum, &pump } ;
Light_ctx lightctx = { &lightMeter, &lightPinNum } ;

Tank_pin tankpin = {&pump , &trig , &echo , &turbid} ;
//end define
...
//    //delay(1000);
    ...
    struct timeval tv ;
    tv.tv_sec = MANUAL_EPOCH_TIME + gmtOffsetSec ;  // apply timezone offset directly
    tv.tv_usec = 0 ;
    settimeofday(&tv, NULL) ;

    Serial.println("Clock set from manual epoch value") ;
    ui_init();
    ...
    lv_obj_add_event_cb(objects.main_page_soil , event_handler , LV_EVENT_CLICKED , NULL) ;
